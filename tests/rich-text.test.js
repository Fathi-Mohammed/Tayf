...
const RICH = {
  blocks: [
    ...
    },
    {
      kind: 'list',
      ordered: false,
      items: [[{ text: 'renew on staging' }], [{ text: 'then production' }]]
    },
...
test('documentFromRich builds the Jira document our editor means', () => {
  ...
  assert.deepEqual(document.content[1].content, [
    ...
    { type: 'hardBreak' },
    { type: 'text', text: 'the spec', marks: [{ type: 'link', attrs: { href: 'https://x.test/s' } }] }
  ]);
  ...
  assert.equal(document.content[4].type, 'blockquote');
  assert.deepEqual(document.content[5], {
    type: 'codeBlock',
...
test('documentFromRich returns null when there is nothing to send', () => {
  ...
  assert.equal(documentFromRich({ blocks: [{ kind: 'list', items: [[]] }] }), null);
});
...
test('a document survives the trip out and back', () => {
  ...
  assert.deepEqual(documentFromRich(back), documentFromRich(RICH));
...
test('richFromDocument reads marks, mentions and lists back into spans', () => {
  ...
  assert.equal(back.blocks[1].spans[5].link, 'https://x.test/s');
  assert.deepEqual(back.blocks[2], {
    kind: 'list',
    ordered: false,
    items: [[{ text: 'renew on staging' }], [{ text: 'then production' }]]
  });
  assert.equal(back.blocks[5].text, 'npm run dist');
});

test('isSupported spots the shapes the editor cannot hold', () => {
  assert.equal(isSupported(documentFromRich(RICH)), true);
  assert.equal(
    isSupported({ type: 'doc', content: [{ type: 'table', content: [] }] }),
    false
  );
  assert.equal(
    isSupported({
  ...
  assert.equal(
    isSupported({ type: 'doc', content: [{ type: 'mediaSingle', content: [] }] }),
    false
  );
});
